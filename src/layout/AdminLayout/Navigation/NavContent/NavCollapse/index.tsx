...
class NavCollapse extends Component<any, any> {
    ...
    render() {
        ...
        const openIndex = isOpen ? isOpen.findIndex(id => id === this.props.collapse.id) : -1;
        if (openIndex > -1) {
        ...
        const triggerIndex = isTrigger ? isTrigger.findIndex(id => id === this.props.collapse.id) : 0;
        if (triggerIndex > -1) {
...
const mapStateToProps = state => {
    return {
        ...
        isOpen: state.layoutReducer.isOpen,
        isTrigger: state.layoutReducer.isTrigger
    }
};
